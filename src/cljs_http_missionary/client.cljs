(ns cljs-http-missionary.client
  (:refer-clojure :exclude [get])
  (:require [cljs-http-missionary.core :as core]
  ...
  (when-not (blank? url)
    (let [uri (uri/parse url)
      ...
      {:scheme (keyword (.getScheme uri))
       ...
       :uri (.getPath uri)
       :query-string (when-not (.isEmpty query-data)
                       (str query-data))
       :query-params (when-not (.isEmpty query-data)
...
(defn- encode-val [k v]
  (str (util/url-encode (name k)) "=" (util/url-encode (str v))))
